document.addEventListener('DOMContentLoaded', () => {
  ...
  // Bind email retry button
  document.getElementById('updates-button-2').addEventListener('click', sendUpdatesEmail);
...
function submitEmailForm() {
  ...
  if (email) {
    // POST to Andrew listserver
    const xhttp = new XMLHttpRequest();
    xhttp.open('POST', 'https://lists.andrew.cmu.edu/mailman/subscribe/endtherain-updates', true);
    xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhttp.send('email=' + encodeURIComponent(email) + '&fullname=&pw=&pw-conf=&digest=0');

    // Hide form and display message
    document.getElementById('updates-form').classList.add('is-hidden');
    document.getElementById('updates-success').classList.remove('is-hidden');
  }
}

// Send email to request mailing list sub
function sendUpdatesEmail() {
  window.location.href = 'mailto:endtherain@gmail.com?subject=Mailing%20List%20Request&body=' + encodeURIComponent(document.getElementById('updates-input').value);
}
